...
bool GameObjectModel::initialize(std::unique_ptr<GameObjectModelOwnerBase> modelOwner, std::string const& dataPath) 
{
    ...
    iPos = Vector3(modelOwner->GetPosition().x, modelOwner->GetPosition().y, modelOwner->GetPosition().z);

	// pussywizard: 
	phasemask = (modelOwner->IsReady() || modelOwner->IsTransport()) ? modelOwner->GetPhaseMask() : 0;
