...
void LoadSkillPerfectItemTable()
{
    ...
    do /* fetch data and run sanity checks */
    {
        ...
        {
            sLog->outErrorDb("Skill perfection data for spell %u has non-existent spell id in `skill_perfect_item_template`!", spellId);
            continue;
        }
        ...
        {
            sLog->outErrorDb("Skill perfection data for spell %u has non-existent required specialization spell id %u in `skill_perfect_item_template`!", spellId, requiredSpecialization);
            continue;
        }
        ...
        {
            sLog->outErrorDb("Skill perfection data for spell %u has impossibly low proc chance in `skill_perfect_item_template`!", spellId);
            continue;
        }
        ...
        {
            sLog->outErrorDb("Skill perfection data for spell %u references non-existent perfect item id %u in `skill_perfect_item_template`!", spellId, perfectItemType);
            continue;
        }
...
struct SkillExtraItemEntry
{
    ...
    SkillExtraItemEntry(uint32 rS, float aCC, int32 nMOE)
        : requiredSpecialization(rS), additionalCreateChance(aCC), additionalMaxNum(nMOE) {}
};
...
void LoadSkillExtraItemTable()
{
    ...
    do
    {
        ...
        uint8 additionalMaxNum = fields[3].GetInt8();
        if (!additionalMaxNum)
        {
